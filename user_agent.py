...
def act(state: OmokState):
    ...
    #         y_pos = np.random.randint(-1, 2)
    #         if x_pos != 0 or y_pos != 0:
    #             break
    #     return 9 + y_pos, 9 + x_pos
...
def alphabeta(state: OmokState, depth, a, b, maximizingPlayer):
    if depth == 0 or state.num_stones == state.board_size * state.board_size:
        ...
        if state.turn == 1:
            my_last_stone = state.history[-2]  # 마지막 흑 돌 좌표 튜플
            opponent_last_stone = state.history[-1]  # 마지막 백 돌 좌표 튜플
        if state.turn == -1:
            my_last_stone = state.history[-1]  # 마지막 흑 돌 좌표 튜플
            opponent_last_stone = state.history[-2]  # 마지막 백 돌 좌표 튜플
